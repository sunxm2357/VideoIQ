...
def load_sound(data_dir, record, idx, fps, audio_length, resampling_rate,
               window_size=10, step_size=5, eps=1e-6, audio_threshold=0):
    import librosa
    ...
    if audio_threshold > 0:
        ...
        if sample_mean < audio_threshold:
            return [Image.fromarray(np.zeros((256, 256 * int(audio_length / 1.28))))]
...
class VideoDataSetLMDB(data.Dataset):
    ...
    def __init__(self, unused, db_path, num_groups=16, frames_per_group=1, sample_offset=0, num_clips=1,
                 modality='rgb', dense_sampling=False, fixed_offset=True,
...
class MultiVideoDataSetLMDB(data.Dataset):
    ...
    def __init__(self, root_path, list_file, num_groups=64, frames_per_group=1, sample_offset=0, num_clips=1,
                 ...
                 fps=29.97, audio_length=1.28, resampling_rate=24000, audio_threshold=0):
        """
        # root_path, modality and transform become list, each for one modality
...
class MultiVideoDataSetOnline(data.Dataset):
    ...
    def __init__(self, root_path, list_file, num_groups=64, frames_per_group=1, sample_offset=0, num_clips=1,
        ...
        if modality == 'rgb' or modality == 'rgbdiff':
            ...
            raise ValueError('Unknow modality = %s' % modality)

        video_datasets = video_dataset_cls(root_path, list_file_,
                                num_groups, frames_per_group, sample_offset,
                                num_clips, modality, dense_sampling, fixed_offset,
                                image_tmpl, transform, is_train, test_mode, seperator,
                                filter_video, num_classes)
        ...
        self.video_list = video_datasets.video_list
        self.num_consecutive_frames = self.video_datasets.num_consecutive_frames
    ...
    def remove_data(self, idx):
        self.video_datasets.remove_data(idx)
        self.video_list = self.video_datasets.video_list
    ...
    def __getitem__(self, index):
        ...
        video_dataset = self.video_datasets
        if indices is None:
            if modality != 'flow':
                record = video_dataset.video_list[index]
            else:
