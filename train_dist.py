...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if 'pact' not in args.backbone_net:
        ...
        clip_val_group['lr'] = args.q_lr[0]
        sgd_polices = [base_group, clip_val_group]
    ...
    if args.backbone_net == 'resnet_pact_swap':
        if args.review_epochs[0] != 0:
            args.review_epochs = [0] + args.review_epochs
        if args.review_epochs[-1] != args.epochs:
            args.review_epochs = args.review_epochs + [args.epochs]

    for epoch in range(args.start_epoch, args.epochs):
        if args.backbone_net == 'resnet_pact_swap':
            for r_idx, r in enumerate(args.review_epochs):
                if r > epoch:
                    break
            current_start = args.review_epochs[r_idx - 1]
            current_end = args.review_epochs[r_idx]
            p = float(epoch - current_start) / float(current_end - current_start) * (1 - args.p_start) + args.p_start
            kwargs = {'p': p}
        else:
            kwargs = {}
        ...
        train_top1, train_top5, train_losses, train_speed, speed_data_loader, train_steps = \
            train(train_loader, model, train_criterion, optimizer, epoch + 1, args,
                  ...
                  use_sparsity=use_sparsity, sparsity_w=sparsity_w, kwargs=kwargs)
        if args.distributed:
            dist.barrier()

        # evaluate on validation set
        val_top1, val_top5, val_losses, val_speed = validate(val_loader, model, val_criterion, args,
                                                             gpu_id=args.gpu)
